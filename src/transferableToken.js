...
class TransferableToken {
    constructor(transactionProvider, contractOwner, keyStore, contractAddress, accountPassword = "", defaultGasLimit = 100000, defaultGasPrice = 1000000000) {
    ...
    deployContract(contractOwner, symbol = "SET", tokenName = "Transferable Meetup token", gasLimit = 900000, gasPrice = 2000000000) {
        const self = this;
        this.contractOwner = contractOwner;
    ...
    issueTokens(toAddress, amount, externalId, reason, gasLimit = this.defaultGasLimit, gasPrice = this.defaultGasPrice) {
        const self = this;
    ...
    async processTransaction(hash, description, gasLimit) {
        ...
        const rawTransactionReceipt = await this.transactionProvider.getTransactionReceipt(hash);
